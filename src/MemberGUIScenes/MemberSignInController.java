					...
					try (Connection con = ConnectDB.CreateConnection();
						...
						while (rs.next()) {
							...
							if (rs.getString("email").equals(txtFieldEmail.getText())
								...
								if (invoice_table.lastEndDate(txtFieldEmail.getText()).isAfter(LocalDate.now())
										|| invoice_table.lastEndDate(txtFieldEmail.getText()).equals(LocalDate.now())) {
									...
									controller.addingData(rs.getString("name"), rs.getString("email"),
											...
											rs.getString("address"), rs.getInt("height"), rs.getInt("weight"),
											rs.getString("medical_data"));
									window.setTitle(rs.getString("email"));
									window.show();
									countForEmailPassAlert = 0;
									break;
								} else {
									new Alert(Alert.AlertType.ERROR, "Your membership plan has expired.", ButtonType.OK)
											.showAndWait();
									countForEmailPassAlert = 0;
									break;
								}
