...
public class invoice_table {
    ...
    public static LocalDate lastEndDate(String email) {
        ...
        String sql = String.format("SELECT invoice.end_date FROM invoice LEFT JOIN member ON member.id = invoice.member_id WHERE member.email = '%s' ORDER BY invoice.id DESC LIMIT 1;", email);
        try (Connection con = ConnectDB.CreateConnection();
                PreparedStatement ps = con.prepareStatement(sql)) {
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
